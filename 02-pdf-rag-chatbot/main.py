...
# 2번. vector 저장소에서 문서를 검색하고 답변을 생성
def retrieve_and_generate_answers(vectorstore, message, temperature=0):
    ...
    #ChatModel 인스턴스 생성
    model = ChatGroq(model="llama-3.3-70b-versatile", temperature=temperature, api_key=GROQ_API_KEY)

    #Prompt 와 ChatModel을 Chain으로 연결
...
demo = gr.ChatInterface(fn=process_pdf_and_answer, 
                        ...
                        gr.Number(value=1000, label="Chunk_size"),
                        gr.Number(value=200, label="Chunk_overlap"),
                        gr.Dropdown(["cosine", "l2"],label="similariy metric", value="cosine"),

...
def execute_and_show_chart(python_code, df):
    try:
        ...
        #예외 발생시 적절한 에러 메세지 반환
        print(f"Eroor executing chart code: {e}")
    return None

def process_and_display(question,csv_file):
...
with gr.Blocks() as demo:
     gr.Markdown(" # CSV파일을 업로드하고, 질문을 입력하세요. 분석결과를 확인 할 수 있습니다")
     with gr.Row():
         csv_input = gr.File(label="csv 파일업로드", type="filepath")
     ...
     submit_button.click(fn = process_and_display,
                         inputs =[question_input, csv_input],
